					...
					<div class="text-center">
						...
						<ul class="store_hours_list">
							<li v-if="storeHours" v-for="hour in storeHours" class="col-xs-12">
								...
								<span v-else class="col-xs-6 text-left"><span v-if="hour.open_time | moment('mm', timezone) == '00'">{{hour.open_time | moment("h mm A", timezone)}}</span><span v-else>{{hour.open_time | moment("h A", timezone)}}</span> - {{hour.close_time | moment("h A", timezone)}}</span>
							</li>
						</ul>
					</div>
				</div>
            ...
            watch: {
                currentStore: function() {
                    if ( _.includes(this.currentStore.store_front_url_abs, 'missing')) {
                        this.currentStore.store_front_url_abs = "//codecloud.cdn.speedyrails.net/sites/5a8c43eb6e6f641a29020000/image/png/1518554684072/bonniedoonlogo.png";
                    }
                    var vm = this;
                    var temp_promo = [];
                    var temp_job = [];
